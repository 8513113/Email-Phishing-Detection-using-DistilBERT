...
# Define oversampling strategy: Match phishing emails (1) to safe emails (0)
safe_email_count = df['Email Type'].value_counts()[0]  # Full dataset count of 0s
oversampler = RandomOverSampler(sampling_strategy={1: safe_email_count}, random_state=42)

# Apply oversampling to the entire dataset
x_resampled, y_resampled = oversampler.fit_resample(df[['Email Text']], df['Email Type'])

# Convert back to DataFrame
df_balanced = pd.DataFrame({'Email Text': x_resampled['Email Text'], 'Email Type': y_resampled})

# Print class distribution after oversampling
print("\nClass distribution after oversampling:")
print(df_balanced['Email Type'].value_counts())

# Split the balanced dataset into training (70%) and test (30%)
train_texts, test_texts, train_labels, test_labels = train_test_split(
    df_balanced['Email Text'], df_balanced['Email Type'], test_size=0.3, random_state=42
)

# Convert back to DataFrame
df_train_balanced = pd.DataFrame({'Email Text': train_texts, 'Email Type': train_labels})
df_test_balanced = pd.DataFrame({'Email Text': test_texts, 'Email Type': test_labels})

# Print class distributions to verify
print("\nBalanced Training set class distribution:")
print(df_train_balanced['Email Type'].value_counts())

print("\nBalanced Test set class distribution:")
print(df_test_balanced['Email Type'].value_counts().sort_index())
